...
with open("questions-words.txt") as data:
    for count,line in enumerate(data,1):
        x=line.split(' ')
        if x[0]!=':':
            valid_text_data.append(line.split(' '))

sents = [' '.join(item) for item in valid_text_data]
...
def bias(word1, embeddings):
    ...
    for cnt in range(1,10):
        for i,item in enumerate(embeddings):
            if cosine_similarity(embedded_vector_1,item)> max_value[cnt] and not np.array_equal(item,embedded_vector_1):
                if i not in max_index:
                    max_value[cnt] = cosine_similarity(embedded_vector_1,item)
                    max_index[cnt] = i
        arr.append(num_to_word[max_index[cnt]])
    return arr
...
for batch_size in [16]:
    for neg_samples in [20]:
        for embedding_size in [128]:
            embeddings = tf.Variable(tf.random_uniform([vocab_size, embedding_size], -1.0, 1.0))
            nce_weights = tf.Variable(tf.random_normal([vocab_size, embedding_size],stddev=1.0 / math.sqrt(embedding_size)))
            nce_biases = tf.Variable(tf.zeros([vocab_size]))
            train_pair1_ind = tf.placeholder(tf.int64, shape=[None,])
            train_pair2_ind = tf.placeholder(tf.int64, shape=[None,1])
            ...
            optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.67).minimize(loss)
            ...
            JOIN = [' '.join(map(str,item)) for item in text]
            
            file = open(batch_size.__str__()+"_"+neg_samples.__str__()+"_"+embedding_size.__str__()+".txt","w")
...
vectors = model.fit_transform(RUN)

fig, ax = plt.subplots()
for word in vocab:
    print(word, vectors[word_to_num[word]][1])
    ax.annotate(word, (vectors[word_to_num[word]][0],vectors[word_to_num[word]][1]), color ='blue')
plt.show()
